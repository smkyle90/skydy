...
# As it stands, the mass and moments of inertia for a Body are defined symbolically, but
# arbitrary. The mass and inertia matrices are MassMatrix and InertiaMatrix objects
# respectively. Let's see.
...
# from this fixed point. We need to instantiate the Ground body and its location. These are special classes.
b_gnd = Ground()
p_gnd = GroundCoordinate()

# We now have to "connect" our car to the ground, via a joint.
# A joint is not necessarily a physical object, it merelely defines the degrees of
# freedom that exists between two bodies. IN this case, the "joint" between the ground
# and the vehicle is a degree of freedom in the body x-coordinate of the car, applied
# at the centre of mass of the car. Thus, define the relative position of the joint in the car frame.
p_car = BodyCoordinate("G1/O", 0, 0, 0)

# We now ready to stipulate what Degrees of Freedom we want to pass through our joint.
# In this case, we are constrained to motion in the x-coordinate, as such, there is
# a DOF in the 0th index (note, indices are 0 through 5 for body coordinates x, y, z, theta_x, theta_y, theta_z).
...
# A Connection is what will allow us to perform our forward kinematics, i.e., get global
# representation of the positions and rotations of each connected body.
# It is critical the the first connection's input body is the ground.
...
oned_car = MultiBody([cnx_car], "1d")

# Draw the system
oned_car.draw(save_fig=True)
